...
export default function Ragdoll(
  ...
  this.createColliders = function () {
    ...
    for (let i = 0; i < this.config.length; i++) {
      let boneNames = this.config[i].hasOwnProperty("bone")
        ? [this.config[i].bone]
        : this.config[i].bones;

      for (let ii = 0; ii < boneNames.length; ii++) {
        let currentBone = this.skeleton.bones[
          this.skeleton.getBoneIndexByName(boneNames[ii])
        ];
        ...
        currentBoxProps.joint = this.config[i].hasOwnProperty("joint")
          ? this.config[i].joint
          : this.defaultJoint;
        currentBoxProps.rotationAxis = this.config[i].hasOwnProperty(
          "rotationAxis"
        )
          ? this.config[i].rotationAxis
          : BABYLON.Axis.X;
        currentBoxProps.min = this.config[i].hasOwnProperty("min")
          ? this.config[i].min
          : this.defaultJointMin;
        currentBoxProps.max = this.config[i].hasOwnProperty("max")
          ? this.config[i].max
          : this.defaultJointMax;

        // Offset value.
        let boxOffset = 0;
        if (
          (this.config[i].hasOwnProperty("putBoxInBoneCenter") &&
            this.config[i].putBoxInBoneCenter) ||
          ...
          if (currentBone.length === undefined) {
            console.log(
              "The length property is not defined for bone",
              currentBone.name,
              ". putBox(es)InBoneCenter will not work :("
            );
          }
        ...
        // Offset axis.
        let boneOffsetAxis = this.config[i].hasOwnProperty("boneOffsetAxis")
          ? this.config[i].boneOffsetAxis
          : this.boneOffsetAxis;
        let boneDir = currentBone.getDirection(boneOffsetAxis, this.mesh);
        currentBoxProps.boneOffsetAxis = boneOffsetAxis;

        box.position = currentBone
          .getAbsolutePosition(this.mesh)
          .add(boneDir.scale(boxOffset));

        let mass = this.config[i].hasOwnProperty("mass")
          ? this.config[i].hasOwnProperty("mass")
          : this.mass;
        let restitution = this.config[i].hasOwnProperty("restitution")
          ? this.config[i].hasOwnProperty("restitution")
          : this.restitution;
        ...
        this.initialRotation.push(
          currentBone.getRotationQuaternion(BABYLON.Space.WORLD, this.mesh)
        );
      }
    }
  };
  ...
  this.initJoints = function () {
    ...
    for (let i = 0; i < this.bones.length; i++) {
      ...
      let distanceFromParentBoxToBone = this.bones[i]
        .getAbsolutePosition(this.mesh)
        .subtract(this.boxes[boneParentIndex].position);

      let wmat = this.boxes[boneParentIndex].computeWorldMatrix();
      let invertedWorldMat = BABYLON.Matrix.Invert(wmat);
      distanceFromParentBoxToBone = BABYLON.Vector3.TransformCoordinates(
        this.bones[i].getAbsolutePosition(this.mesh),
        invertedWorldMat
      );
      ...
      if (this.mainPivotSphereSize != 0) {
        let mainPivotSphere = new BABYLON.MeshBuilder.CreateSphere(
          "mainPivot",
          { diameter: this.mainPivotSphereSize },
          this.scene
        );
  ...
  this.addImpostorRotationToBone = function (boneIndex) {
    const newRotQuat = this.impostors[
      boneIndex
    ].object.rotationQuaternion.multiply(this.initialRotation[boneIndex]);
    this.bones[boneIndex].setRotationQuaternion(
      newRotQuat,
      BABYLON.Space.WORLD,
      this.mesh
    );
  ...
  this.defineRootBone = function () {
    ...
    if (skeletonRoots.length != 1) {
      console.log(
        "Ragdoll creation failed: there can only be 1 root in the skeleton :("
      );
      return false;
    }
    ...
    if (this.rootBoneIndex == -1) {
      console.log(
        "Ragdoll creation failed: the array boneNames doesn't have the root bone in it :( - the root bone is",
        this.skeleton.getChildren()
      );
      return false;
    }
  ...
  this.findNearestParent = function (boneIndex) {
    ...
    do {
      if (
        nearestParent != null &&
        this.boneNames.includes(nearestParent.name)
      ) {
        break;
      }
  ...
  this.init = function () {
    ...
    this.syncBonesAndBoxes = () => {
      if (this.ragdollMode) {
        const rootBoneDirection = this.bones[this.rootBoneIndex].getDirection(
          this.boxConfigs[this.rootBoneIndex].boneOffsetAxis,
          this.mesh
        );
        const rootBoneOffsetPosition = this.bones[this.rootBoneIndex]
          .getAbsolutePosition(this.mesh)
          .add(
            rootBoneDirection.scale(
              this.boxConfigs[this.rootBoneIndex].boxOffset
            )
          );
        ...
        let dist = rootBoneOffsetPosition.subtract(
          this.impostors[this.rootBoneIndex].object.position
        );
        ...
        if (!this.disableBoxBoneSync) {
          ...
          for (let i = 0; i < this.bones.length; i++) {
            this.impostors[i].syncImpostorWithBone(
              this.bones[i],
              this.mesh,
              null,
              this.boxConfigs[i].boxOffset,
              null,
              this.boxConfigs[i].boneOffsetAxis
            );
