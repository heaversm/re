import { Vector3, Quaternion, Matrix, Axis, Space } from '@babylonjs/core/Maths/math';
...
export default function Ragdoll(skeleton, mesh, config, jointCollisions = false, showBoxes = false, mainPivotSphereSize = 0, disableBoxBoneSync = false, isKinematic = false) {
  ...
  this.createColliders = function() {
    ...
    for (let i = 0; i < this.config.length; i++) {
      ...
      for (let ii = 0; ii < boneNames.length; ii++) {
        ...
        currentBoxProps.rotationAxis = this.config[i].hasOwnProperty("rotationAxis") ? this.config[i].rotationAxis : Axis.X;
        ...
        this.initialRotation.push(currentBone.getRotationQuaternion(Space.WORLD, this.mesh));
      }
    }
  }
  ...
  this.initJoints = function() {
    ...
    for (let i = 0; i < this.bones.length; i++) {
      ...
      let invertedWorldMat = Matrix.Invert(wmat);
      distanceFromParentBoxToBone = Vector3.TransformCoordinates(this.bones[i].getAbsolutePosition(this.mesh), invertedWorldMat);
      ...
      let joint = new PhysicsJoint(this.boxConfigs[i].joint, {
      ...
      if (this.mainPivotSphereSize != 0) {
        let mainPivotSphere = new MeshBuilder.CreateSphere("mainPivot", { diameter: this.mainPivotSphereSize }, this.scene);
  ...
  this.addImpostorRotationToBone = function(boneIndex) {
    ...
    this.bones[boneIndex].setRotationQuaternion(newRotQuat, Space.WORLD, this.mesh);
  }
      ...
      if (this.ragdollMode) {
        ...
        if (!this.disableBoxBoneSync) {
          ...
          for (let i = 0; i < this.bones.length; i++) {
            ...
            this.impostors[i].setAngularVelocity(new Vector3());
            this.impostors[i].setLinearVelocity(new Vector3());
          }
        }
      }
    };
  ...
  this.ragdoll = function() {
    ...
    for (let i = 0; i < this.bones.length; i++) {
      ...
      let rotationAdjust = Quaternion.Inverse(this.initialRotation[i]);
